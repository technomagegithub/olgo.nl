...
class Mirasvit_SeoSitemap_Model_Image extends Mage_Core_Model_Abstract
{
    protected $_width;
	protected $_height;
	...
	public function setBaseFile($file)
	{
		...
		$path = array(Mage::getSingleton('mstcore/media_config')->getBaseMediaPath());
  ...
  //   	    $path[] = $this->getDestinationSubdir();
		...
		//AdBlock browser extention blocks content with "ad*" in URL
                $md5 = md5($file.Mage::app()->getStore()->getId()."{$this->_width}x{$this->_height}");
                $md5 = preg_match("/ad[A-Za-z0-9]/i", substr($md5, 0, 3)) ? substr($md5, 1, 3) : substr($md5, 0, 3);
                $path[] = $md5;
                
		$path[] = $this->getUrlKey().'.'.$path_info['extension'];
