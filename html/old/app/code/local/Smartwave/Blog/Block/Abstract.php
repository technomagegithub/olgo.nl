...
abstract class Smartwave_Blog_Block_Abstract extends Mage_Core_Block_Template
{
    ...
    protected static $_catUriParam = Smartwave_Blog_Helper_Data::CATEGORY_URI_PARAM;

    protected static $_postUriParam = Smartwave_Blog_Helper_Data::POST_URI_PARAM;

    protected static $_tagUriParam = Smartwave_Blog_Helper_Data::TAG_URI_PARAM;
    ...
    protected function _processCollection($collection, $category = false)
    {
        ...
        foreach ($collection as $item) {
            ...
            if ($shortContent) {
                ...
                $strManager = new Smartwave_Blog_Helper_Substring(
                    array(
    ...
    {
        if (self::$_helper->conf(Smartwave_Blog_Helper_Data::XML_BOOKMARKS)) {
            return $this->setTemplate('blog/bookmark.phtml')->setPost($post)->renderView();
        }
    }
    ...
    public function getTagsHtml($post)
    {
        if (trim($post->getTags())) {
            return $this->setTemplate('blog/line_tags.phtml')->setPost($post)->renderView();
        }
    }
    ...
    {
        return '<a class="more-link" href="' . $item->getAddress() . '">' . '<span class="button button-custom read-more">' . $this->__('Read More') .'</span>' . '</a>';
    }
    ...
    {
        return Mage::app()->getRequest()->getModuleName() == Smartwave_Blog_Helper_Data::DEFAULT_ROOT;
    }
