...
class Smartwave_Blog_Manage_BlogController extends Mage_Adminhtml_Controller_Action
{
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            if(isset($_FILES['image']['name']) and (file_exists($_FILES['image']['tmp_name']))) {
                ...
                try {
                    ...
                    $path = Mage::getBaseDir('media') . DS . 'wysiwyg' . DS . 'smartwave' . DS. 'blog' . DS;
                               
                    $uploader->save($path, $_FILES['image']['name']);

                    $data['image'] = 'wysiwyg/smartwave/blog/' . preg_replace('/[^a-z0-9_\\-\\.]+/i', '_',$_FILES['image']['name']);
                }catch(Exception $e) {
            ...
            if(isset($_FILES['banner']['name']) and (file_exists($_FILES['banner']['tmp_name']))) {                                   
                try {
                    ...
                    $banneruploader = new Varien_File_Uploader('banner'); 
                    $banneruploader->setAllowedExtensions(array('jpg','jpeg','gif','png')); // or pdf or anything
                    

                    $banneruploader->setAllowRenameFiles(false);

                    $banneruploader->setFilesDispersion(false);

                    $path = Mage::getBaseDir('media') . DS . 'wysiwyg' . DS . 'smartwave' . DS. 'blog' . DS;                    
                    $banneruploader->save($path, $_FILES['banner']['name']);                    
                    $data['banner'] = 'wysiwyg/smartwave/blog/' . preg_replace('/[^a-z0-9_\\-\\.]+/i', '_',$_FILES['banner']['name']); 
