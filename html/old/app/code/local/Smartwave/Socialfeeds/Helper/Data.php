...
class Smartwave_Socialfeeds_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    function fetch_fb_fans($fb_id, $limit = 10){
        ...
        foreach ($peopleList as $key => $code) {
            ...
            if ($link != "") {
                $html .= "<a href=\"".$link."\" title=\"".$name."\" target=\"_blank\"><img src=\"".$img_in_base64."\" alt=\"\" /><span class='fb-name'>".$name."</span></a>";
            } else {
                $html .= "<span title=\"".$name."\"><img src=\"".$img_in_base64."\" alt=\"\" /><span class='fb-name'>".$name."</span></span>";
            }
            $html .= '</div>';
        }
        return $html;
