...
class Smartwave_Granada_Model_Import_Cms extends Mage_Core_Model_Abstract
{
	private $_importPath;
    ...
    {
        parent::__construct();
		$this->_importPath = Mage::getBaseDir() . '/app/code/local/Smartwave/Granada/etc/import/';
    }
	...
	public function importCms($modelString, $itemContainerNodeString, $overwrite = false)
    {
		try
		{
			...
			{
				throw new Exception(
					Mage::helper('granada')->__("Can't get the data file for import cms blocks/pages: %s", $xmlPath)
                );
			}
			...
			if ($i)
			{
				Mage::getSingleton('adminhtml/session')->addSuccess(
					Mage::helper('granada')->__('%s item(s) was(were) imported.', $i)
				);
			}
			else
			{
				Mage::getSingleton('adminhtml/session')->addNotice(
					Mage::helper('granada')->__('No items were imported')
				);
			}
