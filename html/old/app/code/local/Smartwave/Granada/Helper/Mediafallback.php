 ...
 */
class Smartwave_Granada_Helper_Mediafallback extends Mage_ConfigurableSwatches_Helper_Mediafallback
{
	public function getConfigurableImagesFallbackArrayCustom(Mage_Catalog_Model_Product $product, array $imageTypes,
                ...
                if ($imagePath) {
                    $imagesByLabel[$map['label']]['configurable_product']
                        [Mage_ConfigurableSwatches_Helper_Productimg::MEDIA_IMAGE_TYPE_SMALL] =
                            $this->_resizeProductImageCustom($product, 'small_image', $keepFrame, $imagePath, false, $imgWidth, $imgHeight);

                    $imagesByLabel[$map['label']]['configurable_product']
                        [Mage_ConfigurableSwatches_Helper_Productimg::MEDIA_IMAGE_TYPE_BASE] =
                            $this->_resizeProductImageCustom($product, 'image', $keepFrame, $imagePath, false, $imgWidth, $imgHeight);
                }
            }
            ...
            foreach ($imageTypes as $imageType) {
                ...
                if ($product->hasChildrenProducts()) {
                    foreach ($product->getChildrenProducts() as $childProduct) {
                        if ($image = $this->_resizeProductImageCustom($childProduct, $imageType, $keepFrame,null, false, $imgWidth, $imgHeight)) {
                ...
                if ($image = $this->_resizeProductImageCustom($product, $imageType, $keepFrame, null, true, $imgWidth, $imgHeight)) {
     ...
     * @return string|bool
     */
    protected function _resizeProductImageCustom($product, $type, $keepFrame, $image = null, $placeholder = false, $imgWidth = null, $imgHeight = null)
    {
        ...
        if ($hasTypeData || $placeholder || $image) {
			...
			if ($keepFrame) {
				...
				$helper->constrainOnly(FALSE)->resize($imgWidth, $imgHeight);
			} else {
				$helper->constrainOnly(FALSE)->resize($imgWidth);
			}
