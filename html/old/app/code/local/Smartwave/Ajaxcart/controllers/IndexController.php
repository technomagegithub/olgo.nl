		...
		if($params['isAjax'] == 1){
			...
			try {
				...
				if (!$cart->getQuote()->getHasError()){
					...
					$this->loadLayout();
                    $header_cart = "";
                    if($this->getLayout()->getBlock('header.cart.sidebar'))
                        $header_cart = $this->getLayout()->getBlock('header.cart.sidebar')->toHtml();
                    $top_cart = "";
                    if($this->getLayout()->getBlock('header.top.cart.sidebar'))
                        $top_cart = $this->getLayout()->getBlock('header.top.cart.sidebar')->toHtml();
                    $icon_cart="";
                    if($this->getLayout()->getBlock('header.mini.icon.cart'))
                        $icon_cart = $this->getLayout()->getBlock('header.mini.icon.cart')->toHtml();

					Mage::register('referrer_url', $this->_getRefererUrl());
                    $response['header_cart'] = $header_cart;
                    $response['top_cart'] = $top_cart;
                    $response['icon_cart'] = $icon_cart;
		...
		try {
            ...
            */
            ...
            if ($root) {
				...
				//$root->addBodyClass(Mage::getStoreConfig('legenda_design/general/theme_style',$code));
